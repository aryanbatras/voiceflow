...
class InfiniteGridMenu {
  ...
  public onItemClick?: (itemIndex: number) => void;
  ...
  public setItems(newItems: MenuItem[]): void {
    if (newItems.length !== this.items.length) {
      this.rebuildPositions(newItems.length);
      this.initDiscInstances(this.DISC_INSTANCE_COUNT);
    }
    this.items = newItems;
  }

  private rebuildPositions(count: number): void {
    ...
    for (let i = 0; i < n; i++) {
      ...
      this.instancePositions.push(
        vec3.fromValues(
          Math.cos(theta) * radiusAtY * this.SPHERE_RADIUS,
          y * this.SPHERE_RADIUS,
          Math.sin(theta) * radiusAtY * this.SPHERE_RADIUS
        )
      );
    }
    // Swap the position closest to (0, 0, -R) to index 0 so first feed faces camera
    if (n > 1) {
      let frontIdx = 0;
      let bestDot = -Infinity;
      const frontDir = vec3.fromValues(0, 0, -1);
      for (let i = 0; i < n; i++) {
        const dir = vec3.normalize(vec3.create(), this.instancePositions[i]);
        const d = vec3.dot(dir, frontDir);
        if (d > bestDot) {
          bestDot = d;
          frontIdx = i;
        }
      }
      const tmp = this.instancePositions[0];
      this.instancePositions[0] = this.instancePositions[frontIdx];
      this.instancePositions[frontIdx] = tmp;
    }
  ...
  private init(onInit?: InitCallback): void {
    ...
    this.rebuildPositions(this.items.length);
    this.initDiscInstances(this.DISC_INSTANCE_COUNT);
    this.initTexture();
    ...
    this.canvas.addEventListener('pointerup', (e) => {
      ...
      if (elapsed < 300 && !pointerMoved && this.onItemClick) {
  ...
  private loadTextureImage(url: string): Promise<HTMLImageElement> {
    return new Promise<HTMLImageElement>(resolve => {
      const img = new Image();
      img.onload = () => resolve(img);
      img.onerror = () => {
        const fb = new Image();
        fb.src = 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512"><rect fill="#222" width="512" height="512"/></svg>');
  ...
  private initTexture(): void {
    ...
    const entries = this.items.map(item => this.loadTextureImage(item.image));
    Promise.all(entries).then(images => {
      ...
      if (images.length < this.atlasSize * this.atlasSize) {
        const emptyIndex = images.length;
        const x = (emptyIndex % this.atlasSize) * cellSize;
        const y = Math.floor(emptyIndex / this.atlasSize) * cellSize;
        ctx.fillStyle = '#111';
...
const defaultItems: MenuItem[] = [
  {
    image: 'data:image/svg+xml,' + encodeURIComponent('<svg xmlns="http://www.w3.org/2000/svg" width="512" height="512"><rect fill="#222" width="512" height="512"/></svg>'),
    link: '/',
    title: 'Loading...',
    description: '',
  }
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0, onItemClick }) => {
  ...
  useEffect(() => {
    ...
    const handleActiveItem = (index: number) => {
      ...
      if (itemIndex !== lastActiveIndex) {
        ...
        const label = currentItems[itemIndex]?.title || '';
        document.title = `Immersive — ${label}`;
      }
    };
    ...
    return () => {
      ...
      currentSketch && (() => { /* cleanup handled by GC */ })();
    };
  }, [scale]);
  ...
  useEffect(() => {
    if (menuRef.current && items.length > 0) {
      ...
      menuRef.current.refreshTexture();
      setActiveItem(items[0]);
    }
  }, [items]);

  useEffect(() => {
    if (!menuRef.current || !onItemClick) return;
    const currentItems = items;
    menuRef.current.onItemClick = (index: number) => {
      if (currentItems[index]) onItemClick(currentItems[index], index);
    };
  }, [onItemClick, items]);
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <div
          className={`absolute bottom-0 left-0 right-0 z-10 px-8 pb-10 pt-16 bg-gradient-to-t from-black/90 via-black/60 to-transparent transition-all ease-[cubic-bezier(0.25,0.1,0.25,1.0)] ${
            isMoving
              ? 'opacity-0 pointer-events-none duration-100 translate-y-4'
        ...
        >
          <div className="max-w-5xl mx-auto flex items-end justify-between gap-6">
            <div className="flex-1 min-w-0">
              {activeItem.category && (
                <span className="inline-block text-xs font-medium text-white/50 mb-1.5 tracking-wider uppercase">
                  {activeItem.category}
                </span>
              )}
              <h2 className="text-white text-[clamp(1.25rem,2.5vw,2.25rem)] font-bold leading-tight truncate">
                {activeItem.title}
              </h2>
              <p className="text-white/60 text-[clamp(0.75rem,1.2vw,1rem)] mt-1 line-clamp-2 max-w-xl">
                {activeItem.description}
              </p>
              {activeItem.likeCount !== undefined && (
                <p className="text-white/40 text-sm mt-1.5">
                  {(activeItem.likeCount / 1000).toFixed(1)}k likes
                </p>
              )}
            </div>
            <button
              onClick={handleButtonClick}
              className="shrink-0 px-6 py-3 rounded-full bg-white text-black font-semibold text-sm hover:bg-white/90 transition-all active:scale-95"
            >
              Go
            </button>
          </div>
