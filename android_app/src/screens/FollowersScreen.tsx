...
import { getFollowers } from '@/services/graph';
...
export default function FollowersScreen({ navigation, route }: any) {
  ...
  const fetchData = useCallback(async (c?: string): Promise<{ items: any[]; cursor?: string }> => {
    ...
    try {
      const result = await getFollowers(handle, c);
      return result;
    } catch {
    ...
    return (
      <SafeAreaView className="flex-1 bg-background">
        <View className="flex-row items-center px-4 py-3 border-b border-border">
          ...
          <Text className="text-lg font-bold text-foreground ml-3">Followers</Text>
        </View>
        <View className="flex-1 items-center justify-center">
          <Text className="text-muted-foreground">Sign in to view followers</Text>
        </View>
      </SafeAreaView>
    );
  }
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <View className="py-16 items-center">
              <UserPlus size={40} color="#8e8e8e33" />
              <Text className="text-muted-foreground text-lg mt-3">No followers yet</Text>
            </View>
          }
        />
      )}
